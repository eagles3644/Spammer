...
public class GMailSender {
    ...
    public SMTPTransport connectToSmtp(String host, int port, String userEmail,
        ...
        Log.v("GMailSender", "came to connecttosmtp");
        ...
        Log.v("GMailSender", "came before gen response");
        byte[] response = String.format("user=%s\1auth=Bearer %s\1\1", userEmail, oauthToken).getBytes();
        response = BASE64EncoderStream.encode(response);

        Log.v("GMailSender", "came to call issuecommand " + transport.isConnected());
        Log.v("GMailSender", new String(response));

        transport.issueCommand("AUTH XOAUTH2 " + new String(response), 235);

        Log.v("GMailSender", "came after issue command");
        return transport;
    }
